...
>
> **WKTW re-open (2026-07-04) — recall regressed on a fresh full run.** The 2026-07-01 fix was verified by re-running against the *old* `20260624` workspace, where the opener strings were fixed. On the fresh `audit-weknowthewhy-20260702-12a9972b` crawl (routing fix already deployed — tickets emitted 2026-07-02, a day after the 07-01 fix), `healthy-checks.md` caught only 6 while **6 passing-check guardrail tickets leaked back into `deliverables/tickets/`**:
> - `inp-excellent-no-issues` — *"Codify the structural conditions producing 24ms INP … as an enforceable Astro layout contract"*
> - `api-network-waterfall-minimal` — *"Document and enforce the zero-runtime-fetch architectural invariant …"*
> - `dom-quality-excellent` — *"Preserve the lightweight DOM baseline …"*
> - `ghost-markup-no-orphaned-assets` — *"Institutionalize the current clean CSS/JS architecture …"*
> - `stability-5-content-visibility-not-needed`
