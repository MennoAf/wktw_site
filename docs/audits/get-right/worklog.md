...
| `0f9f728` | 2026-06-26 | WL-21, WL-18 (search + social share) | Build-time link/sitemap validation (`scripts/check-links.mjs`, gates deploys); Pagefind site search (`/search` + nav links) and social-share buttons, both behind `src/data/features.ts` flags; CSP gains `'wasm-unsafe-eval'` for Pagefind. |
...
- **Status:** 🟢 Foundation done; **search + social-share SHIPPED** (feature-flagged); related-content deferred (content-volume-gated) · **Closed by:** `e198ec7` (content-model foundation) + `0f9f728` (search + social share)
- **Update (2026-06-26, Jason):** the two content-model-*independent* features — **Pagefind site search** and **social-share buttons** — were built to satisfy the audit's `navigation-no-search` and `conversion-ux-social-share-1` checks ahead of the re-audit. Both sit behind `src/data/features.ts` flags (`search`, `socialShare`) so they can be flipped off once the re-audit records them. **RelatedContent stays deferred** — with one article there's nothing to relate to; it's a small add once article #2 lands (reads `src/data/insights.ts`).
  - **Search:** Pagefind indexes built HTML (`pagefind --site dist` in the `build` script, scoped via `data-pagefind-body` on `<main>`). New `/search` page + header/footer "Search" links. CSP edge function gained `'wasm-unsafe-eval'` (Pagefind runs WASM) — WASM-compile only, not JS eval. Verified: 17 pages indexed, assets emitted to `dist/pagefind/`, page wired.
  - **Social share:** `src/components/SocialShare.astro` — pure X/LinkedIn/Email anchor links (no JS, no CSP surface) on the article.
...
- **Status:** ☑ Done (repo) — **live validation + enforce flip pending (ops)** · **Closed by:** `fb0a84d` (static headers) + `6053c1c` (CSP). `6053c1c` replaces the toothless static report-only CSP (it kept `'unsafe-inline'`, so it could never report a violation) with a **per-request nonce-based CSP** minted in a Netlify Edge Function (`netlify/edge-functions/csp.ts`). It nonces every `<script>` (inline, module, external Plausible, ld+json) and emits a `'strict-dynamic'` policy so the nonced GTM loader can pull in `gtm.js`/GA4 without host-allowlisting Google's rotating URLs. Static headers (XFO, nosniff, Referrer-Policy, HSTS, Permissions-Policy) remain in `netlify.toml`.
  ...
  - **⚠️ Before flipping `CSP_MODE=enforce` (on a deploy preview):** (1) load several pages, confirm **zero CSP violations** in the console / Netlify function logs in report-only mode; (2) accept cookies → confirm **GTM + GA4 fire** (strict-dynamic propagation) and **Plausible** loads; (3) submit the contact + scan forms → `/thanks` `generate_lead` still pushes; (4) exercise the mobile menu + consent banner (their scripts are nonced); (5) **load `/search` and run a query → results render** (Pagefind WASM relies on the `'wasm-unsafe-eval'` grant added for WL-18). Only then set `CSP_MODE=enforce`. Also still confirm no plain-HTTP subdomain before trusting HSTS `includeSubDomains`.
...
- **Status:** ☑ Done · **Closed by:** `0f9f728` (2026-06-26) — `scripts/check-links.mjs` runs as a postbuild step (wired into `npm run build`, so it gates Netlify deploys). It crawls every `dist/**/*.html`, hard-fails (exit 1) on any internal `<a href>`/asset reference with no matching file, and validates the sitemap: index + shards present, non-empty, every `<loc>` resolves, and `/scan` is **not** leaked. Fragment (`#id`) mismatches are warnings, not failures (anchors can be JS-injected). Self-contained — no third-party link-checker dependency. Verified both ways: passes clean on the real build (17 pages), fails with exit 1 on an injected dead link.
...
- **Files:** `scripts/check-links.mjs`, `package.json` (build script)
...
- **Action:** Handed to our **analytics lead** (manages the container) via **[`gtm-investigation.md`](./gtm-investigation.md)** — a standalone, console-facing checklist. Confirm whether a standalone GA4 config tag AND a separate gtag fire; if the dual-beacon exists, fix it **in GTM**, not the repo. Findings get logged in that doc; closes here once the console review reports back.

### 🎛️➕ WL-23 — Conversion event instrumentation
- **Status:** 🟢 Repo half done (GTM tag still console-side) · **Closed by:** `60b32b2` (2026-06-25) — `/thanks` fires a `generate_lead` dataLayer push (`form_type` read from live URL at runtime: `contact` | `scan`). Self-gates on consent. Validated in-browser. **Remaining (GTM-side):** create the GA4 tag/trigger that consumes `generate_lead` in container GTM-5VQTG6TH — added to `gtm-investigation.md` for the console review.
...
| --- | --- | --- |
| 🟡 WL-24 | `ux-sitemap-unverifiable` | `@astrojs/sitemap` already installed & configured with `/scan` excluded (`astro.config.mjs:3,9`). **Now actively verified** at build time by WL-21's `check-links.mjs` (sitemap present, non-empty, every `<loc>` resolves, `/scan` absent). |
...
- 2026-06-25 — **All repo-actionable items closed** (WL-01–12 + WL-19/20 + WL-23 repo half). Consent gate live-validated in a real browser (14/14 behaviors: GTM never loads pre-consent / after Decline; fires on Accept; fonts fully self-hosted — no gstatic/googleapis). Remaining work is GTM-console (analytics lead: WL-22, WL-23 GA4 tag), additive design passes (WL-13–18), and the enforcing nonce-CSP follow-up (WL-20).
...
- 2026-06-26 — **WL-21 closed + WL-18 visible features shipped** `0f9f728`: build-time link + sitemap validation (`scripts/check-links.mjs`, postbuild, gates Netlify deploys; verified it fails on a broken link and passes clean). Pagefind site search (`/search` + header/footer links, `data-pagefind-body`-scoped index) and social-share buttons shipped behind `src/data/features.ts` flags so they can be flipped off after the re-audit — built to satisfy `navigation-no-search` + `conversion-ux-social-share-1`. CSP edge function gained `'wasm-unsafe-eval'` (Pagefind WASM); WL-20 enforce-flip checklist updated to verify search. **WL-18 RelatedContent still deferred** (needs article #2). Toolchain `npm audit` (4 advisories, all pre-existing Astro deps, none exploitable on a static Mac/Linux/Netlify build) handled separately as a hygiene commit.
- 2026-06-25 — **WL-18 content-model decided + foundation built** `e198ec7`: chose a typed metadata manifest (`src/data/insights.ts`) over Astro content collections — one bespoke article today, so collections would be premature abstraction; the real friction (listing↔article metadata duplication) is now fixed, behavior-preserving. Visible WL-18 features (related-content, Pagefind search, social share) **deferred until content volume justifies them**. Remaining across the audit: **WL-13 deferred** (trust signals — no testimonials yet); WL-18 visible features (content-gated); optional WL-21 (build-time link-checking); ops-only WL-20 enforce flip; GTM-console WL-22 + WL-23 GA4 tag (analytics lead).
